...
func TestCacheEntries(t *testing.T) {
	...
	clk.Add(time.Duration(DefaultCacheNegLocalTTL/2) * time.Second)
	l.Put(questionMsg3, nil, DefaultCacheNegLocalTTL, CacheNoLocalReplies)
	clk.Add(time.Duration(1) * time.Second)
	l.Put(questionMsg3, nil, DefaultCacheNegLocalTTL, CacheNoLocalReplies)
	clk.Add(time.Duration(1) * time.Second)
	l.Put(questionMsg3, nil, DefaultCacheNegLocalTTL, CacheNoLocalReplies)
	...
	clk.Add(time.Duration(DefaultCacheNegLocalTTL/2) * time.Second)
	t.Logf("Checking that we get an expired response after %f seconds", DefaultCacheNegLocalTTL)
