...
func (router *NetworkRouter) handleCapturedPacket(key PacketKey) FlowOp {
	...
	router.learnMAC(key, router.Ourself.Peer, true)
...
func (router *NetworkRouter) handleForwardedPacket(key ForwardPacketKey) FlowOp {
	...
	router.learnMAC(key.PacketKey, key.SrcPeer, false)
...
func (router *NetworkRouter) learnMAC(key PacketKey, srcPeer *mesh.Peer, isLocalMAC bool) {
	srcMac := net.HardwareAddr(key.SrcMAC[:])

	switch newSrcMac, conflictPeer := router.Macs.Add(srcMac, srcPeer); {
	case newSrcMac && isLocalMAC:
		log.Print("Discovered local MAC ", srcMac)
	case newSrcMac:
		log.Print("Discovered remote MAC ", srcMac, " at ", srcPeer)
	case conflictPeer != nil:
		...
		if isLocalMAC {
			...
			log.Print("Discovered remote MAC ", srcMac, " at ", srcPeer, " (was at ", conflictPeer, ")")
		}
