...
> one easily with
> [kubeadm](http://kubernetes.io/docs/getting-started-guides/kubeadm/). Alternatively if you are using the older cluster set-up scripts from
...
cluster, you must first uninstall it before applying the Weave-kube addon. Shut down Kubernetes, and _on all nodes_ perform the following:
